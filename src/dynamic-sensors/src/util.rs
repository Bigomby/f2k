...
pub fn v6_to_v4(netmask: &IpAddr) -> IpAddr {
    match netmask {
        ...
        &IpAddr::V6(netmask) => {
            let ref octets = netmask.octets();
            IpAddr::from(Ipv4Addr::new(octets[12], octets[13], octets[14], octets[15]))
        }
    }
...
pub fn apply_netmask(ip: &IpAddr, netmask: &IpAddr) -> IpAddr {
    match (ip, netmask) {
        (&IpAddr::V4(ip), &IpAddr::V4(netmask)) => {
            ...
            IpAddr::from(Ipv4Addr::from(ipv4_to_u32(ip_octets) & ipv4_to_u32(netmask_octets)))
        }
        (&IpAddr::V6(ip), &IpAddr::V6(netmask)) => {
            let ref ip_octets = ip.octets();
            let ref netmask_octets = netmask.octets();
            IpAddr::from(Ipv6Addr::from(ipv6_to_u128(ip_octets) & ipv6_to_u128(netmask_octets)))
        }
        _ => panic!("Trying to apply a v6 netmask to an v4 address"),
    }
}
